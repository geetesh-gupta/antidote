...
init([]) ->
    % DistMonEnabled = app_helper:get_env(antidote, enable_dist_mon,
    %                                     true),
    %{ok, Root} = application:get_env(antidote, platform_data_dir),
...
ensembles_enabled() ->
    ...
    Enabled = app_helper:get_env(antidote, enable_consensus, false),
    Exists and Enabled.
