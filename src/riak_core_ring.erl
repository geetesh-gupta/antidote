...
check_tainted(Ring=?CHSTATE{}, Msg) ->
    Exit = app_helper:get_env(antidote, exit_when_tainted, false),
...
fresh(NodeName) ->
    fresh(app_helper:get_env(antidote, ring_creation_size), NodeName).
...
pretty_print(Ring, Opts) ->
    ...
    TargetN = proplists:get_value(target_n, Opts,
                                  app_helper:get_env(antidote, target_n_val)),
