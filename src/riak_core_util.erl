...
enable_job_class(Class) ->
    case app_helper:get_env(antidote, job_accept_class) of
        [_|_] = EnabledClasses ->
            case lists:member(Class, EnabledClasses) of
                ...
                _ ->
                    application:set_env(
                        antidote, job_accept_class, [Class | EnabledClasses])
            end;
        _ ->
            application:set_env(antidote, job_accept_class, [Class])
    end.
        ...
        [_|_] = EnabledClasses ->
            case lists:member(Class, EnabledClasses) of
                false ->
                    ok;
                _ ->
                    application:set_env(antidote, job_accept_class,
...
determine_all_n(Ring) ->
    ...
    Default = app_helper:get_env(antidote, default_bucket_props),
