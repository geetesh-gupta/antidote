...
vnode_modules() ->
    case application:get_env(antidote, vnode_modules) of
        undefined -> [];
        {ok, Mods} -> Mods
    end.

bucket_fixups() ->
     case application:get_env(antidote, bucket_fixups) of
        undefined -> [];
        {ok, Mods} -> Mods
    end.

bucket_validators() ->
    case application:get_env(antidote, bucket_validators) of
        undefined -> [];
        {ok, Mods} -> Mods
    end.

stat_mods() ->
    case application:get_env(antidote, stat_mods) of
        undefined -> [];
        {ok, Mods} -> Mods
    end.

health_check(App) ->
    case application:get_env(antidote, health_checks) of
        undefined ->
            undefined;
        {ok, Mods} ->
...
register_mod(App, Module, Type) when is_atom(Type) ->
    ...
    case application:get_env(antidote, Type) of
        undefined ->
            application:set_env(antidote, Type, [{App,Module}]);
        {ok, Mods} ->
            application:set_env(antidote, Type,
...
register_metadata(App, Value, Type) ->
    case application:get_env(antidote, Type) of
        undefined ->
            application:set_env(antidote, Type, [{App,Value}]);
        {ok, Values} ->
            application:set_env(antidote, Type,
...
register_proplist({Key, Value}, Type) ->
    case application:get_env(antidote, Type) of
        undefined ->
            application:set_env(antidote, Type, [{Key, Value}]);
        {ok, Values} ->
            application:set_env(antidote, Type, lists:keystore(Key, 1,
                                                                Values,
                                                                {Key, Value}))
    end.
