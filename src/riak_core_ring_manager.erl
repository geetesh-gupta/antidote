...
do_write_ringfile(Ring) ->
    case ring_dir() of
        ...
        Dir ->
            ...
            Cluster = app_helper:get_env(antidote, cluster_name),
...
reload_ring(live) ->
    case riak_core_ring_manager:find_latest_ringfile() of
        ...
        {error, Reason} ->
            lager:critical("Failed to load ring file ~p: ~p",
                           [ring_dir(), lager:posix_error(Reason)]),
            throw({error, Reason})
    end.
...
ring_dir() ->
    case app_helper:get_env(antidote, ring_state_dir) of
        undefined ->
            filename:join(app_helper:get_env(antidote, platform_data_dir, "data"), "ring");
        D ->
            D
    end.
...
%% process.
set_ring_global(Ring) ->
    DefaultProps = case application:get_env(antidote, default_bucket_props) of
        {ok, Val} ->
            Val;
        _ ->
            []
...
set_ring_global_test() ->
    setup_ets(test),
    application:set_env(antidote,ring_creation_size, 4),
...
refresh_my_ring_test() ->
    {spawn, fun() ->
        ...
        Core_Settings = [{ring_creation_size, 4},
                         ...
                         {cluster_name, "test"}],
        [begin
             put({?MODULE,AppKey}, app_helper:get_env(antidote, AppKey)),
             ok = application:set_env(antidote, AppKey, Val)
        ...
        [ok = application:set_env(antidote, AppKey, get({?MODULE, AppKey}))
