...
init(_State) ->
    ...
    {Tokens, _} = app_helper:get_env(antidote, gossip_limit, ?DEFAULT_LIMIT),
...
schedule_next_reset() ->
    {_, Reset} = app_helper:get_env(antidote, gossip_limit, ?DEFAULT_LIMIT),
...
attempt_simple_transfer(Seed, Ring, Owners, ExitingNode) ->
    TargetN = app_helper:get_env(antidote, target_n_val),
